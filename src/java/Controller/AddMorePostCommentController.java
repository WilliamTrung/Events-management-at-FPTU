 ...
 * @author SE151264
 */
public class AddMorePostCommentController extends HttpServlet {

    private final String ERROR = "error.jsp";
    private final String SUCCESS = "LoadMorePostCommentController";
    /**
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            PostDTO post= (PostDTO) session.getAttribute("SELECTED_POST");
            Date cmtDatetime= Date.valueOf(LocalDate.now());
            CommentDTO cmt= new CommentDTO(comment, user, cmtDatetime);
            
            if(post!=null){
                if(dao.insertPostComment(cmt, post.getPostId())){
                    url=SUCCESS;
                }else{
